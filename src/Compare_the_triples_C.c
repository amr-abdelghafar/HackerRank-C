...
int min_length = 0;
int length_offset = abs(a_count - b_count);

if (length_offset != 0) {
    if (a_count < b_count ){
        min_length = a_count;
        bob_count += length_offset;
    } else {
        min_length = b_count;
        alice_count += length_offset;
    }    
} else {
    min_length = a_count;
}
...
int indx = 0;
for (indx = 0; indx < min_length; indx++){
    if (a[indx] > b[indx]){
        alice_count++;
    }else if (b[indx] > a[indx]){
        bob_count++;
    }
}

result[0] = alice_count;
result[1] = bob_count;
*result_count = 2;
